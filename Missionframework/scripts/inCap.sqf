...
// Variable ob ein "dragger" gefunden wurde, zum verlassen der Schleife
private _foundDragger = false;
///////////////////Find dragger loop////////////////////////////////////////
while {true} do {
    // Am Anfang von jedem Durchlauf gucken, ob im Bereich von 50m eine Person vorhanden ist.
    _nearestunits = _unit nearEntities ["CAManBase", 50];
    // Prüfe, ob überhaupt ein Dragger gefunden werden kann (wenn keine "nearEntities" da sind, wird dragger nichts zugewiesen und somit kommt es zu Fehlern)
    if ((count (_nearestunits - (playableUnits + switchableUnits + [_unit]))) > 0) then {
        // Picke dir einen zufälligen aus den gefundenen Leuten raus
        _dragger = selectRandom (_nearestunits - (playableUnits + switchableUnits + [_unit]));
        sleep 1;
        // Ist der gefundene Dragger nun z.B. auch kein Zivilist und auch noch bei Bewusstsein....
        if ((alive _unit) && ((side _dragger) != civilian) && ((lifeState _dragger) != "INCAPACITATED") && (alive _dragger) && ((AnimationState _dragger) != "AcinPknlMwlkSrasWrflDb")) then {
            // ...haben wir unseren Dragger gefunden....
            _foundDragger = true;
        }; 
    };
    // ...und können damit hier die Schleife verlassen
    if (_foundDragger) exitWith {};

    // Ist der "Patient" irgendwann tot, können wir es auch sein lassen...
    if (!alive _unit) exitWith {};
};

// ...und hier dann sogar das komplette Script verlassen
if (!alive _unit) exitWith {}; 
